...
const Index = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#1A1F2C] to-[#2A2A30] text-white font-light">
      {/* Logo with proper z-index - increased size */}
      <div className={`flex justify-center pt-10 mb-6 transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'} z-0`}>
        <Logo />
      </div>

      {/* Main content - AI interface and productions */}
      <div className={`flex-grow flex flex-col items-center justify-center transition-opacity duration-1000 delay-300 ${isLoaded ? 'opacity-100' : 'opacity-0'} z-10`}>
        <OptionButtons />
      </div>
